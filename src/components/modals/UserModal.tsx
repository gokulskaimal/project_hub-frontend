      ...
      <div className="relative bg-white rounded-lg border border-gray-200 shadow-xl w-full max-w-2xl mx-4 p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div className="mb-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-1">
            {isEdit ? `Edit Profile: ${state.computedName}` : `Manage Profile: ${state.computedName}`}
          </h2>
          <p className="text-sm text-gray-600">
            {isEdit ? "Update your profile information and settings" : "View and manage your profile information"}
          </p>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm mb-6">
          <div className="flex items-center gap-4">
            <div className="relative w-16 h-16 bg-gray-100 rounded-full grid place-items-center overflow-hidden">
              {state.profileImage ? (
                <Image src={state.profileImage} alt="Profile" width={64} height={64} className="object-cover w-full h-full" />
              ) : (
                <span className="text-xl font-medium text-gray-900">{state.computedInitial}</span>
              )}
            ...
            <div className="flex-1">
              <h3 className="text-lg font-medium text-gray-900">{state.computedName}</h3>
              <p className="text-gray-600">{state.email}</p>
              <div className="flex gap-2 mt-2">
                 <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold border border-gray-200 text-gray-900">{displayRole}</span>
                 <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-gray-900 text-white">ACTIVE</span>
              </div>
            </div>
            {isEdit && <input ref={refs.fileInputRef} type="file" className="hidden" accept="image/*" onChange={actions.handleImageUpload} />}
          </div>
        </div>
        ...
        {mode === 'view' ? (
          ...
          <EditUserForm
            ...
            setCurrentPassword={(v) => setters.setPasswords(p => ({...p, current: v}))}
            setNewPassword={(v) => setters.setPasswords(p => ({...p, new: v}))}
            setConfirmNewPassword={(v) => setters.setPasswords(p => ({...p, confirm: v}))}
        ...
        <div className="flex justify-end gap-3 mt-6">
          <button onClick={handleClose} className="px-4 py-2 border border-gray-200 rounded-md text-sm font-medium hover:bg-gray-50">Close</button>
          {isEdit && (
            <button 
