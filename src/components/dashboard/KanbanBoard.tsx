...
const COLUMNS = [
  { id: "TODO", title: "To Do", color: "bg-gray-50/50", border: "border-gray-200", borderStyle: "border-dashed", titleColor: "text-gray-700" },
  { id: "IN_PROGRESS", title: "In Progress", color: "bg-blue-50/50", border: "border-blue-200", borderStyle: "border-dashed", titleColor: "text-blue-700" },
  { id: "REVIEW", title: "Review", color: "bg-yellow-50/50", border: "border-yellow-200", borderStyle: "border-dashed", titleColor: "text-yellow-700" },
  { id: "DONE", title: "Done", color: "bg-green-50/50", border: "border-green-200", borderStyle: "border-dashed", titleColor: "text-green-700" },
];
...
import { Play, Square, Clock, Calendar as CalendarIcon, Flag, MoreHorizontal } from "lucide-react";

export default function KanbanBoard({ tasks, users, onStatusChange, onEditTask, showProjectBadges = false }: KanbanBoardProps) {
  ...
  const handleToggleTimer = async (e: React.MouseEvent, task: Task) => {
    e.stopPropagation();
    const activeLog = task.timeLogs?.find(l => l.userId === user?.id && !l.endTime);
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="flex h-full gap-6 overflow-x-auto pb-4 items-start min-h-[500px] px-2">
        {COLUMNS.map((col) => (
            /* Column Container */
          <div key={col.id} className={`flex flex-col h-full min-w-[280px] flex-1 rounded-xl border-2 ${col.border} ${col.borderStyle} ${col.color} p-0`}>
            {/* Column Header */}
            <div className="flex items-center justify-between p-4 pb-2">
                 <h3 className={`font-bold text-sm bg-transparent ${col.titleColor}`}>
                     {col.title}
                 </h3>
                        ...
                        >
                          ...
                          <div className="flex flex-wrap gap-2 mb-4">
                                ...
                                <span className={`text-[10px] px-2 py-0.5 rounded font-medium ${
                                    task.priority === 'CRITICAL' || task.priority === 'HIGH' ? 'bg-red-50 text-red-600' :
                          ...
                          <div className="flex items-center justify-between pt-2 border-t border-gray-50 mt-auto">
                                <div className="flex items-center gap-3">
                                     {/* Flag Icon */}
                                    <Flag size={14} className={`${task.priority === 'CRITICAL' ? 'text-red-500 fill-red-500' : 'text-gray-300'}`} />
                                    
                                     {/* Due Date */}
                                    {task.dueDate && (
                                        <div className="flex items-center gap-1 text-[10px] text-gray-400">
                                            <CalendarIcon size={12} />
                                            <span>{new Date(task.dueDate).toLocaleDateString(undefined, { day: '2-digit', month: '2-digit', year: 'numeric'})}</span>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    {task.assignedTo === user?.id && (
                                        ...
                                        className={`flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-mono transition-colors ${
                                            isTracking ? 'bg-red-50 text-red-600' : 'bg-gray-50 text-gray-400 hover:text-gray-600'
                                        }`}
                                        >
                                            {isTracking ? <Square size={10} fill="currentColor" /> : <Play size={10} fill="currentColor" />}
                                    ...
                                    {task.assignedTo && (
                                        <div 
                                            className="w-6 h-6 rounded-full bg-gradient-to-r from-violet-200 to-pink-200 flex items-center justify-center text-[9px] font-bold text-gray-700 border border-white shadow-sm"
                                            title={getUser(task.assignedTo)?.email}
                                        >
                                            {getUser(task.assignedTo)?.firstName?.charAt(0) || "?"}
                                        </div>
                                    )}
                                </div>
                          </div>
