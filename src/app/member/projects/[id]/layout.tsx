...
export default function MemberProjectLayout({
  ...
  const tabs = [
    { name: "Overview", href: `/member/projects/${projectId}`, icon: LayoutList, exact: true },
    { name: "Board", href: `/member/projects/${projectId}/board`, icon: KanbanSquare, exact: false },
  ];
                     ...
                     <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-3">
                        ...
                        {project && (
                            <span className={`text-[10px] px-2 py-0.5 rounded-full border uppercase tracking-wide ${
                                project.status === 'ACTIVE' ? 'bg-green-100 text-green-600 border-green-100' : 'bg-gray-50 text-gray-500 border-gray-100'
                            }`}>
                                {project.status}
                            </span>
                        )}
