...
import { CheckCircle, CreditCard, RefreshCw, Zap, Shield, Star, Rocket } from "lucide-react";
...
export default function ManagerPlansPage() {
  ...
  const getPlanIcon = (type: string) => {
      switch(type) {
          case 'ENTERPRISE': return <Rocket className="w-6 h-6 text-orange-600" />;
          case 'PRO': return <Zap className="w-6 h-6 text-purple-600" />;
  ...
  const getPlanColor = (type: string) => {
      switch(type) {
          case 'ENTERPRISE': return 'bg-orange-50 border-orange-100 ring-orange-100';
          case 'PRO': return 'bg-purple-50 border-purple-100 ring-purple-100';
          default: return 'bg-blue-50 border-blue-100 ring-blue-100';
      }
  };
      ...
      <div className="space-y-6">
          <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
             <div>
                 ...
                 <p className="text-xs text-gray-500">You are currently on the <span className="font-bold text-blue-600">{currentOrg?.subscriptionStatus || 'Free'}</span> plan</p>
             </div>
             <button 
                onClick={fetchData} 
          ...
          {loading ? (
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                 {[1,2,3].map(i => (
                     <div key={i} className="bg-white rounded-2xl p-6 h-96 animate-pulse border border-gray-100"></div>
                 ))}
             </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {activePlans.map((plan) => {
                    ...
                    return (
                        <div key={plan.id} className={`relative bg-white rounded-2xl border transition-all duration-300 flex flex-col ${isCurrentPlan ? `border-blue-500 ring-2 ring-blue-100 shadow-lg scale-[1.02] z-10` : 'border-gray-200 hover:shadow-xl hover:-translate-y-1'}`}>
                             {isCurrentPlan && (
